export function split_time(time: number): { hours: number, minutes: number, seconds: number } {
...
export function parse_time(time: string): number {
  ...
  return new Date(0, 0, 0, hours, minutes, seconds, 0).getTime() - new Date(0, 0, 0, 0, 0, 0, 0).getTime()
}
export function time_to_string(time: number): string {
  const { seconds, minutes, hours } = split_time(time)
